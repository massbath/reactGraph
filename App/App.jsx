...
import {Grid,Row,Col,Panel,ControlLabel,Table, Form, FormControl, FormGroup,InputGroup,ButtonGroup,Button, Glyphicon, Modal} from 'react-bootstrap';
...
import {DownloadButton,UploadButton,FormGroupWithLabel} from './CommonComponents.jsx';
...
export default class AppWatGenerator  extends React.Component{
  ...
  constructor(props){
    ...
    showDetailLinkSelected(source,target){
      ...
      this.setState({showModalLinkSelected : true, sourceStep : sourceStep, targetStep:targetStep});
    }
    ...
    changeStep(detailStep){
      ...
      for(var i =0 ; i < oldLinks.length; i++){
        var indexIdTarget = detailStep.linkedStepsId.indexOf(oldLinks[i].target.id);
        ...
        else if(oldLinks[i].source.id == nodeSelected.id && indexIdTarget !=-1){
            newLinks.push(oldLinks[i]);
...
deleteStep(nodeSelected){
    ...
    var newLinks = oldState.links.filter(function(link){ return link.source.id != stepToDelete.id && link.target.id != stepToDelete.id });
...
deleteLink(sourceStep, targetStep){
  var newLinks = this.state.links.filter(function(link){return link.source.id != sourceStep.id && link.target.id != targetStep.id});
  this.setState({
      links : newLinks
  });
  this.closeModalLink();
...
render(){
      var valuesLink = {source : this.state.addLinkSourceValue, target :this.state.addLinkTargetValue};
      var stepsLinked = [];
            ...
            <Grid>
              <Row >
                  ...
                  <Panel>
                    <Form>
                      <Col sm={6}>
                        <FormGroupWithLabel label="Charger les étapes"  >
                           <UploadButton onChange={(data) => this.loadSteps(data) } accept=".steps"/>
                        </FormGroupWithLabel>
                      </Col>
                      <Col sm={6}>
                         <FormGroupWithLabel label="Charger une configuration existante"  >
                            <UploadButton onChange={(data) => this.loadConfig(data) } accept=".config"/>
                        </FormGroupWithLabel>
                      </Col>
                    </Form>
                  </Panel>
                    {this.state.stepsOfApplication.length >0 &&
                      <Panel>
                        <Col sm={12} >
                            <AddStepComponent steps={this.state.stepsOfApplication}   onClick ={(stepToAdd)=>this.addStep(stepToAdd)} />
                        </Col>
                    </Panel>
                    }
              </Row>
              <Row>
                {this.state.steps.length > 0 &&
                  <Panel>
                    <Col sm={6}>
                      <GraphConfiguration steps={this.state.steps} links= {this.state.links} selectNodeCallback = {(idStep) => this.showDetailStepSelected(idStep)} selectLinkCallback = {(source,target) => this.showDetailLinkSelected(source,target)}/>
                    </Col>
                    <Col sm={6}>
                      {this.state.links.length > 0 &&
                                <Form horizontal>
                                    <FormGroupWithLabel label="Préfixe des chemins des étapes dans les scénarios"  >
                                        <FormControl name="prefixeSteps" value={this.state.prefixeSteps}  type="text"  onChange={this.handleChange}/>
                                   </FormGroupWithLabel>
                                   <FormGroupWithLabel label="Télécharger">
                                       <DownloadButton  label="La configuration" dataToDownload={this.state} defaultFileName="config.config"/>
                                       {'  '}
                                        <DownloadResult  label="Les scénarios" links={this.state.links} steps={this.state.steps} prefixeSteps={this.state.prefixeSteps}/>
                                   </FormGroupWithLabel>
                                 </Form>
                            }
              ...
              {this.state.showModalLinkSelected &&
                <ModalLink
                    ...
                    deleteLink= {()=>this.deleteLink(this.state.sourceStep,this.state.targetStep)}
                  />
              }
            </div>
