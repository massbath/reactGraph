...
import {Grid,Row,Col,ControlLabel,Table, Form, FormControl, FormGroup,InputGroup,ButtonGroup,Button, Glyphicon} from 'react-bootstrap';
              ...
              <FormGroup>
                      <InputGroup>
                          <Select
                                ...
                                onChange = {(newValue)=>{this.props.onChange('addLinkSourceValue', newValue)}}
                            />
                            <br></br>
                            <Select
                                  options={this.props.steps}
                                  value= {this.props.values.target}
                                  onChange = {(newValue)=>{this.props.onChange('addLinkTargetValue',newValue)}}
                              />
                            <InputGroup.Button>
                                <Button onClick={()=>this.props.onValidation() }>Ajouter lien</Button>
                            </InputGroup.Button>
                        </InputGroup>
                    </FormGroup>
                </div>);
...
export class ListLinksComponent extends React.Component{
  ...
  render(){
    var listLinks = this.props.links.map((input, index) =>
                        <tr key={index}>
                            <td>#({input.source.id}) {input.source.label}</td>
                            <td>#({input.target.id}) {input.target.label}</td>
                            <td>
                                <ActionRowTable
