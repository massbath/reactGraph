...
export class AddStepComponent extends React.Component {
  ...
  render() {
    return (
      <FormGroupWithLabel label="Ajouter une étape à la configuration">
        <Col sm={8}>
          <Select
            options={this.props.steps}
            value={this.state.stepSelected}
            onChange={newValue => {
              this.onChangeStepSelected(newValue)
            }}
          />
        </Col>
        {'  '}
        <Button
          bsStyle="primary"
          onClick={() => this.addValidateStepSelected()}>
          Ajouter étape
        </Button>
      </FormGroupWithLabel>
    )
...
export class ModalStep extends React.Component {
  constructor(props) {
    ...
    this.showConfirmationDeleteNodeModal = this.showConfirmationDeleteNodeModal.bind(
      this
    )
    this.cancelConfirmationDeleteNodeModal = this.cancelConfirmationDeleteNodeModal.bind(
      this
    )
    this.validateConfirmationDeleteNodeModal = this.validateConfirmationDeleteNodeModal.bind(
      this
    )
  }
  ...
  render() {
    ...
    var detailStep = {
      stepId: this.props.stepToShow.id,
      valueStep: this.state.valueStep,
      linkedStepsId: this.state.stepsLinked
    }

    const space = detailStep.linkedStepsId.map(stepId => <br key={stepId} />)

    return (
      <div>
        <Modal
          show={!this.state.showConfirmation}
          onHide={() => this.props.cancelCallback()}
          bsSize="lg">
          ...
          <Modal.Body>
            <FormGroupWithLabel label="Etape">
              <Select
                ...
                name="currentStep"
                onChange={newVal =>
                  this.onChangeStepHandler(newVal, 'valueStep')
                }
              />
            </FormGroupWithLabel>
            <br />
            ...
            <FormGroupWithLabel label="Etape(s) suivante(s)">
              <Select
                ...
                valueKey="id"
                onChange={newVal =>
                  this.onChangeStepHandler(newVal, 'stepsLinked')
                }
              />
            </FormGroupWithLabel>
            <br />
          ...
          <Modal.Footer>
            <Button
              bsStyle="danger"
              onClick={() => this.showConfirmationDeleteNodeModal()}>
              Supprimer l'étape
            </Button>
            <Button
              bsStyle="primary"
              onClick={() => this.props.validateChangeOnNode(detailStep)}>
...
export class ModalLink extends React.Component {
  constructor(props) {
    ...
    this.cancelConfirmationDeleteModal = this.cancelConfirmationDeleteModal.bind(
      this
    )
    ...
    return (
      <div>
        <Modal
          ...
          <Modal.Body>
            Lien entre les étapes <b>{this.props.source.label}</b> et{' '}
            <b>{this.props.target.label}</b>
          </Modal.Body>
          <Modal.Footer>
            <Button
              bsStyle="danger"
              onClick={() => this.showConfirmationDeleteLink()}>
              Supprimer le lien
            </Button>
            <Button onClick={() => this.props.cancelCallback()}>Annuler</Button>
          </Modal.Footer>
